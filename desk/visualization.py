...
def plot_pixel_timeseries(stack, row=None, col=None, period=12):
    ...
    # 原始时序
    axs[0, 0].plot(times, series, 'o-', linewidth=2, markersize=4,
    ...
    try:
        if np.sum(valid_mask) >= 3:
            ...
            coeffs = np.polyfit(valid_times, valid_series, 1)
            trend_line = np.polyval(coeffs, times)

            axs[0, 1].plot(times, series, 'o-', alpha=0.7, label='原始数据')
            axs[0, 1].plot(times, trend_line, '--', linewidth=2,
    ...
    try:
        ...
        if np.sum(valid_mask) >= max(3, period * 2):
            ...
            if not np.all(valid_mask):
                series_filled = np.interp(times, times[valid_mask], series[valid_mask])

            stl_result = STL(series_filled, period=period, robust=True).fit()

            # 趋势分量
            axs[1, 0].plot(times, stl_result.trend, linewidth=2,
            ...
            # 季节分量
            axs[1, 1].plot(times, stl_result.seasonal, linewidth=2,
